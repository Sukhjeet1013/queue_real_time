...
def ensure_column(connection, inspector, table_name, column_name, sql_type):
    ...
    connection.execute(
        text(f"ALTER TABLE {table_name} ADD COLUMN {column_name} {sql_type} NULL")
    )
...
def ensure_postgres_timezone_columns(connection, inspector):
    ...
    for table_name, column_names in column_targets.items():
        ...
        for column_name in column_names:
            ...
            if getattr(column.get("type"), "timezone", False):
                continue
...
def ensure_queue_schema():
    ...
    if "queue_entries" not in tables or "clinics" not in tables:
        return
...
def compute_average_consultation_minutes(clinic_id):
    weighted_minutes = weighted_average(get_recent_consultation_durations(clinic_id))
...
def refresh_clinic_average_consultation_minutes(clinic_id):
    ...
    if rolling_average is None:
        clinic.average_consultation_minutes = (
            clinic.average_consultation_minutes or Config.DEFAULT_CONSULTATION_TIME
        )
    else:
        clinic.average_consultation_minutes = max(
            Config.MIN_CONSULTATION_TIME,
            rolling_average,
        )
...
def build_superadmin_dashboard_context():
    ...
    admins_by_clinic = defaultdict(list)
    for admin_user in admin_users:
        if admin_user.clinic_id:
            admins_by_clinic[admin_user.clinic_id].append(admin_user)

    metrics_rows = db.session.execute(
        select(
            QueueEntry.clinic_id,
            func.count(QueueEntry.id).label("total_entries"),
            func.coalesce(
                func.sum(
                    case((QueueEntry.status == QueueEntry.STATUS_WAITING, 1), else_=0)
                ),
                0,
            ).label("waiting_count"),
            func.min(
    ...
    for clinic in clinics:
        clinic_metrics = metrics_by_clinic.get(
            clinic.id,
            {"total_entries": 0, "waiting_count": 0, "active_token": None},
        )
        total_waiting += clinic_metrics["waiting_count"]
        clinic_rows.append(
            {
                "clinic": clinic,
                "admins": admins_by_clinic.get(clinic.id, []),
                "waiting_count": clinic_metrics["waiting_count"],
                "total_entries": clinic_metrics["total_entries"],
                "has_active_consultation": clinic_metrics["active_token"] is not None,
                "active_token": clinic_metrics["active_token"],
            }
        )
...
def register_cli(app):
    ...
    @app.cli.command("repair-queue")
    def repair_queue_command():
        with app.app_context():
            repair_queue_state()
        print("Queue state repaired successfully.")
...
def register_routes(app):
    ...
    def home():
        ...
        try:
            ...
            log_database_exception("Failed to render landing page.")
    ...
    def clinics_list():
        try:
            ...
            log_database_exception("Failed to render clinics page.")
            return render_template("clinics.html", clinics=[], clinic_count=0)
    ...
    def clinic_page(clinic_id):
        try:
            ...
            log_database_exception(f"Failed to load clinic {clinic_id}.")
            abort(503, description="Clinic data is temporarily unavailable.")
        ...
        try:
            ...
            log_database_exception(f"Failed to render clinic page for clinic {clinic_id}.")
            abort(503, description="Clinic data is temporarily unavailable.")
    ...
    def join_queue():
        ...
        try:
            ...
            log_database_exception("Failed to create queue entry.")
    ...
    def queue_status(entry_id):
        try:
            entry = db.session.execute(
                select(QueueEntry)
                .options(joinedload(QueueEntry.clinic))
                .where(QueueEntry.id == entry_id)
            ).scalar_one_or_none()
        except SQLAlchemyError:
            log_database_exception(f"Failed to load queue entry {entry_id}.")
            abort(503, description="Queue data is temporarily unavailable.")
        ...
        try:
            ...
            log_database_exception(f"Failed to render queue page for entry {entry_id}.")
            abort(503, description="Queue data is temporarily unavailable.")
    ...
    def queue_status_api(entry_id):
        try:
            ...
            log_database_exception(f"Failed to load queue status API for entry {entry_id}.")
            return jsonify({"error": "Queue data is temporarily unavailable."}), 503
        ...
        try:
            ...
            log_database_exception(f"Failed to build queue snapshot for entry {entry_id}.")
            return jsonify({"error": "Queue data is temporarily unavailable."}), 503
        ...
        if request.method == "POST":
            ...
            if not username or not password:
                ...
                try:
                    ...
                    log_database_exception("Failed to load user during login.")
                    error_message = "Login is temporarily unavailable. Please try again shortly."
                else:
    ...
    def admin_dashboard():
        try:
            ...
            log_database_exception("Failed to render admin dashboard.")
            abort(503, description="Admin data is temporarily unavailable.")
    ...
    def add_clinic():
        if request.method == "POST":
            ...
            try:
                ...
                log_database_exception("Failed to create clinic.")
                flash("We could not create the clinic right now. Please try again.", "danger")
                return render_template("add_clinic.html")
    ...
    def assign_admin():
        try:
            ...
            log_database_exception("Failed to load assign admin page.")
            abort(503, description="Admin data is temporarily unavailable.")
        ...
        if request.method == "POST":
            ...
            try:
                ...
                log_database_exception("Failed to load clinic during admin assignment.")
                flash("We could not assign the admin right now. Please try again.", "danger")
            ...
            try:
                ...
                log_database_exception("Failed to validate admin username uniqueness.")
                flash("We could not assign the admin right now. Please try again.", "danger")
                return render_template("assign_admin.html", **context)
            ...
            try:
                ...
                log_database_exception("Failed to assign clinic admin.")
                flash("We could not assign the admin right now. Please try again.", "danger")
                return render_template("assign_admin.html", **context)

            flash(
                f"Clinic admin {username} has been assigned to {clinic.clinic_name}.",
                "success",
            )
            return redirect(url_for("admin_dashboard"))
    ...
    def call_next(clinic_id=None):
        ...
        if effective_clinic_id != current_user.clinic_id:
            abort(403, description="You can only manage your assigned clinic.")

        try:
            clinic = db.session.get(Clinic, effective_clinic_id)
        except SQLAlchemyError:
            log_database_exception("Failed to load clinic during call next.")
            flash("The queue is temporarily unavailable. Please try again.", "danger")
            return redirect(url_for("admin_dashboard"))
        ...
        try:
            active_entry = get_active_entry_for_clinic(effective_clinic_id)
        except SQLAlchemyError:
            log_database_exception("Failed to load active entry during call next.")
            flash("The queue is temporarily unavailable. Please try again.", "danger")
            return redirect(url_for("admin_dashboard"))
        ...
        try:
            next_entry = db.session.scalar(
                select(QueueEntry)
                .where(
                    QueueEntry.clinic_id == effective_clinic_id,
                    QueueEntry.status == QueueEntry.STATUS_WAITING,
                )
                .order_by(QueueEntry.token_number.asc())
                .limit(1)
            ...
            log_database_exception("Failed to load next waiting entry.")
            flash("The queue is temporarily unavailable. Please try again.", "danger")
            return redirect(url_for("admin_dashboard"))
        ...
        try:
            ...
            log_database_exception("Failed to commit call next transition.")
    ...
    def mark_served(entry_id):
        try:
            ...
            log_database_exception(f"Failed to load queue entry {entry_id} during mark served.")
            flash("The queue is temporarily unavailable. Please try again.", "danger")
            return redirect(url_for("admin_dashboard"))
        ...
        try:
            ...
            log_database_exception("Failed to mark patient as served.")
...
def register_error_handlers(app):
    ...
    def handle_500(error):
        db.session.rollback()
        logger.error("Unhandled 500 error: %s", error)
